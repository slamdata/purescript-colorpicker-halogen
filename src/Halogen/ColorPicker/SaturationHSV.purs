module Halogen.ColorPicker.SaturationHSV where
...
component' props = HCP.proxy $ H.component
  { initialState: \color → { color, input: show $ (C.toHSVA color).s }
  , render: render props
  , eval
...
eval = case _ of
  Receive c next → do
    st ← H.get
    unless (st.color == c)
      $ H.put { color: c, input: show $ (C.toHSVA c).s }
    pure next
  Update str next → do
    st ← H.get
    ...
    when (not G.isNaN s && s >= 0.0 && s <= 1.0) do
      ...
      let newColor = C.hsva hsva.h s hsva.v hsva.a
