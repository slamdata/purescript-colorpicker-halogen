module Halogen.ColorPicker.Luminosity where
...
component' props = HCP.proxy $ H.component
  { initialState: \color → { color, input: show $ (C.toHSLA color).l }
  , render: render props
  , eval
...
eval = case _ of
  Receive c next → do
    st ← H.get
    unless (st.color == c)
      $ H.put { color: c, input: show $ (C.toHSLA c).l }
    pure next
  Update str next → do
    st ← H.get
    H.modify (_{ input = str })
    let l = G.readFloat str
    when (not G.isNaN l && l >= 0.0 && l <= 1.0) do
      let hsla = C.toHSLA st.color
      let newColor = C.hsla hsla.h hsla.s l hsla.a
