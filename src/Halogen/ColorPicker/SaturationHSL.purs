module Halogen.ColorPicker.SaturationHSL where
...
component' props = HCP.proxy $ H.component
  { initialState: \color → { color, input: show $ (C.toHSLA color).s }
  , render: render props
  , eval
...
eval = case _ of
  Receive c next → do
    st ← H.get
    unless (st.color == c)
      $ H.put { color: c, input: show $ (C.toHSLA c).s }
    pure next
  Update str next → do
    st ← H.get
    H.modify (_{ input = str })
    let s = G.readFloat str
    when (not G.isNaN s && s >= 0.0 && s <= 1.0) do
      let hsla = C.toHSLA st.color
      let newColor = C.hsla hsla.h s hsla.l hsla.a
