...
component' props = HCP.proxy $ H.component
  { initialState: \color → { color, input: show $ (C.toHSVA color).h }
  , render: render props
  , eval
...
eval = case _ of
  Receive c next → do
    st ← H.get
    unless (st.color == c)
      $ H.put { color: c, input: show $ (C.toHSVA c).h }
    pure next
  Update str next → do
    st ← H.get
    H.modify (_{ input = str })
    let h = G.readFloat str
    when (not G.isNaN h) do
      let hsva = C.toHSVA st.color
      let newColor = C.hsva h hsva.s hsva.v hsva.a
